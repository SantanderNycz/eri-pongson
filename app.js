...
function preload() {
  ...
  barra1Img = loadImage("./assets/barra.png");
  barra2Img = loadImage("./assets/barra.png");
  bounceSound = loadSound(
    "./assets/shooting.mp3",
    () => console.log("bounceSound carregado com sucesso"),
    (err) => console.error("Erro ao carregar bounceSound:", err)
  );
}

function tocarSomColisao() {
  if (bounceSound && bounceSound.isLoaded()) {
    bounceSound.play();
  } else {
    console.warn("Som de colisão não carregado.");
  }
}
...
class Bola {
  ...
  reiniciar() {
    ...
    } while (this.velocidadeX === 0);

    do {
      this.velocidadeY = random(-3, 3);
    } while (this.velocidadeY === 0);
  }
